...
# レシーバは配列を想定。
 def to_sentence(options = {})
  ...
  # connectorとは主に接続部分を表す。今回は配列の間の接続言葉を示す。
    ...
    # もし引数optionsにkeyのlocaleが存在する場合 かつ I18nに定義されている場合
    # I18nとは簡単にいうと、言語の変換機能
    if options[:locale] != false && defined?(I18n)
      # support以下のarray部分の言語を引数optionsに存在する言語（:locale）に翻訳する。 翻訳したものをハッシュ型でi18n_connectors変数へ格納。
      i18n_connectors = I18n.translate(:'support.array', locale: options[:locale], default: {})
      # デフォルトの接続言葉(default)に上記の翻訳(i18n)バージョンをくっつける(mergeする)。！がついているため値が変更される。（破壊的メソッドであるため。）
      default_connectors.merge!(i18n_connectors)
    end
    # options変数へ引数のハッシュをマージ（破壊的）する。
    # keyが重複する場合、後に追加したkeyが優先される。すなわち、引数optionsにdefaultと重複するkeyが存在する場合、
    # 引数optionsのvalueが優先される。
